module PunditBot
  class Dataset
    ...
    def data_claims(hash_of_election_results, politics_condition)
      trues, falses = data.to_a.partition.each_with_index{|val, idx| hash_of_election_results[val[0]] } #TODO: factor out; but something like it is used in data_claims
      ...
      data_claims = {
        ...
        :numeric => [
          ...
          DataClaim.new( lambda{|x, yr| x > data[(yr.to_i-1).to_s] },
            {
              :v => 'grow',
              :tense => :past,
          ...
          DataClaim.new( lambda{|x, yr| x > data[(yr.to_i-politics_condition.election_interval).to_s] }, 
            {
              :v => 'grow',
              :tense => :past,
              # TODO: this is actually a complement
